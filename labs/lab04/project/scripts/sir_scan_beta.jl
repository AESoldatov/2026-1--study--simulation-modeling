...
function run_experiment(p)  # Создаём β_und и β_det на основе скалярного beta
    beta = p[:beta]
    β_und = fill(beta, 3)
    β_det = fill(beta/10, 3) # Передаём в модель
    ...
    for step = 1:p[:n_steps] # Ручной шаг (безопасный)
